...
class UserApi extends Api {
	...
	public function show() {
		...
		$map ['toUserId'] = $uid;
		$map ['status'] = 1;
		$gifts = M ( 'gift_user' )->field ( 'id,fromUserId,toUserId,giftPrice,giftImg' )->where ( $map )->limit ( 4 )->order ( 'id DESC' )->findAll (); // giftId,giftName,giftNum,
		$gift_list = array ();
		...
		$user_info ['tags'] = ( array ) model ( 'Tag' )->setAppName ( 'public' )->setAppTable ( 'user' )->getAppTags ( $uid );
		
		return $user_info;
	}

	//获取用户勋章
	function get_user_medal() {
	...
	public function user_follower() {
		...
		foreach ( $follower as $k => $v ) {
			...
			$follower_arr [$k] ['uname'] = $follower_info ['uname'];
			$follower_arr [$k] ['intro'] = $follower_info ['intro'] ? $follower_info ['intro'] : '';
			$follower_arr [$k] ['avatar'] = $follower_info ['avatar'] ['avatar_big'];
	...
	public function user_following() {
		...
		foreach ( $following as $k => $v ) {
			...
			$following_arr [$k] ['intro'] = $following_info ['intro'] ? $following_info ['intro'] : '';
	...
	public function user_friend() {
		...
		foreach ( $friend as $k => $v ) {
			...
			$friend_arr [$k] ['intro'] = $friend_info ['intro'] ? $friend_info ['intro'] : '';
	...
	public function user_friend_by_letter() {
		...
		if (! t ( $this->data ['key'] )) { // 无搜索
			foreach ( $friend as $k => $v ) {
				...
				$letters [$first_letter] [$v ['follow_id']] ['intro'] = $friend_info ['intro'] ? $friend_info ['intro'] : '';
				$letters [$first_letter] [$v ['follow_id']] ['avatar'] = $friend_info ['avatar'] ['avatar_big'];
			...
			foreach ( $user as $k => $v ) {
				...
				$user_detail ['intro'] = $friend_info ['intro'] ? $friend_info ['intro'] : '';
				$user_detail ['avatar'] = $friend_info ['avatar'] ['avatar_big'];
	...
	public function user_blacklist() {
		...
		foreach ( $list as $k => $v ) {
			...
			$user_blacklist [$k] ['intro'] = $blacklist_info ['intro'] ? $blacklist_info ['intro'] : '';
	...
	public function do_bind_phone() {
		...
		if (! $smsDao->checkLoginCode ( $phone, $code )) {
			return array (
					'status' => 0,
					'msg' => $smsDao->getError () 
			);
		...
		$result = model ( 'User' )->where ( $map )->setField ( 'login', $phone );
		if ($result !== false) {
			model ( 'User' )->cleanCache ( $this->mid );
			return array (
					'status' => 1,
	...
	public function user_bind() {
		...
		$tel_bind [0] ['name'] = '手机号';
		$login = model ( 'User' )->where ( 'uid=' . $this->mid )->getField ( 'login' );
		if (preg_match ( "/^[1][358]\d{9}$/", $login ) !== 0) {
			$tel_bind [0] ['isBind'] = 1;
		} else {
			$tel_bind [0] ['isBind'] = 0;
		}
	...
	public function unbind() {
		...
		if ($type == 'phone') {
			...
			$res = model ( 'User' )->where ( 'uid=' . $this->mid )->setField ( 'login', $uname );
			if ($res !== false) {
				model ( 'User' )->cleanCache ( $this->mid );
				return array (
						'status' => 1,
