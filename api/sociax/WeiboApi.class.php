...
class WeiboApi extends Api {
	...
	public function recommend_timeline() {
		...
		$where = "is_del=0 and is_audit=1 and is_recommend=1 and app='public'";
		! empty ( $max_id ) && $where .= " AND a.feed_id < {$max_id}";
	...
	public function denounce_weibo() {
		...
		if ($isDenounce = model ( 'Denounce' )->where ( $data )->count ()) {
			...
			if ($id = model ( 'Denounce' )->add ( $data )) {
				...
				model ( 'Credit' )->setUserCredit ( $this->mid, 'reported_weibo' );
		...
		if (! $key) {
			...
			if ($user_list) {
				foreach ( $user_list as $k => $v ) {
					...
					$at_list [$k] ['intro'] = $v ['intro'] ? $v ['intro'] : '';
